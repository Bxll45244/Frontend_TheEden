...
import Header from "../../components/Starter/Header.jsx"; // ตรวจสอบ Path ให้ถูกต้องตามโครงสร้างไฟล์ของคุณ
...
  // *** เพิ่ม state สำหรับเก็บ Authentication Token ***
  // ในการใช้งานจริง Token นี้ควรจะได้มาจากการ Login ของผู้ใช้ และเก็บไว้ใน localStorage หรือ context API
  const [authToken, setAuthToken] = useState(null); 

  // useEffect สำหรับดึง Token จาก localStorage (ตัวอย่าง)
  useEffect(() => {
    // ในการใช้งานจริง คุณจะต้องมีกระบวนการ Login และบันทึก Token
    const storedToken = localStorage.getItem('userToken'); // สมมติว่าเก็บ Token ไว้ใน 'userToken'
    if (storedToken) {
      setAuthToken(storedToken);
    } else {
      // ถ้าไม่มี Token อาจจะ redirect ไปหน้า Login หรือแสดงข้อความเตือน
...
  // และเมื่อ authToken เปลี่ยน (ในกรณีที่มีการ login/logout)
  useEffect(() => {
    if (authToken) { // ดึงข้อมูลเฉพาะเมื่อมี Token แล้ว
      fetchHoleStatuses();
    } else if (!isLoading && !authToken && error === null) {
        // ถ้าไม่มี Token และยังไม่ได้โหลดข้อมูล (หรือโหลดเสร็จแล้วแต่ไม่มี Token)
        // ตั้งค่า error เพื่อแสดงผลว่าต้อง login
        setError("กรุณาเข้าสู่ระบบเพื่อดูสถานะหลุมกอล์ฟ");
        setIsLoading(false);
    }
  }, [authToken]); // เพิ่ม authToken ใน dependency array
...
        // *** เพิ่ม Headers สำหรับ Authentication Token ***
        headers: {
          Authorization: `Bearer ${authToken}`, // ส่ง Token ในรูปแบบ Bearer
        },
      });
      const backendData = response.data; 

      // ตรวจสอบโครงสร้างข้อมูลที่มาจาก Backend อีกครั้ง
      if (!backendData || !Array.isArray(backendData.holeStatuses)) {
...
        setError("การเข้าถึงไม่ได้รับอนุญาต กรุณาเข้าสู่ระบบใหม่");
...
    if (!confirmData) return;
    if (!authToken) { // ตรวจสอบ Token ก่อนส่ง request
        setPopup({ title: "คุณไม่ได้เข้าสู่ระบบ กรุณาเข้าสู่ระบบก่อนทำรายการ", isError: true });
        setConfirmData(null);
        return;
    }
...
        // สำหรับ action ที่ไม่มี API backend รองรับ ยังคงอัปเดตสถานะ Frontend เท่านั้น
...
      let errorMessage = `เกิดข้อผิดพลาดในการ ${title}`;
      if (err.response) {
        if (err.response.status === 401) {
          errorMessage = "การเข้าถึงไม่ได้รับอนุญาต กรุณาเข้าสู่ระบบใหม่";
          // ตัวอย่าง: หากเป็น 401 ให้ล้าง Token และ redirect
